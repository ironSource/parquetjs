...
function shredRecordInternal(schema, record, data, rlvl, dlvl) {
  for (let colDef of schema.columns) {
    ...
    if (record[k].constructor === Array && colDef.repetition_type == 'REPEATED') {
      for (let i = 0; i < record[k].length; ++i) {
        data[k].push({
          value: parquet_types.toPrimitive(colType, record[k][i]),
      ...
      data[k].push({
        value: parquet_types.toPrimitive(colType, record[k]),
        rlevel: rlvl,
        dlevel: colDef.dLevelMax,
      });
    }
...
exports.materializeRecords = function(schema, buffer) {
  ...
  for (let i = 0; i < buffer.rowCount; ++i) {
    ...
    for (let colDef of schema.columns) {
      ...
      // FIXME don't modify
      let dlevel = buffer.columnData[colPath].dlevels.shift();
      let rlevel = buffer.columnData[colPath].rlevels.shift();

      if (dlevel == colDef.dLevelMax) {
        record[colDef.name] = parquet_types.fromPrimitive(
            colDef.originalType,
            buffer.columnData[colPath].values.shift());
