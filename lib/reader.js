...
function decodeDataPages(buffer, opts) {
  ...
  while (cursor.offset < cursor.size) {
    ...
    Array.prototype.push.apply(
        data,
        decodeDataPage(cursor, pageHeader, opts));
  }
...
function decodeDataPage(cursor, header, opts) {
  let rLevels = parquet_codec_rle.decodeValues(
      "INT32",
      cursor,
      header.data_page_header.num_values,
      { bitWidth: parquet_util.getBitWidth(opts.rLevelMax) });

  let dLevels = parquet_codec_rle.decodeValues(
      "INT32",
      cursor,
      header.data_page_header.num_values,
  ...
  switch (valueEncoding) {
    ...
    case 'PLAIN':
      values = parquet_codec_plain.decodeValues(
          opts.type,
          cursor,
          valueCount,
          {});
