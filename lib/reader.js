...
class ParquetEnvelopeReader {
  ...
  async readHeader() {
    let buf = await this._vf.read(0, PARQUET_MAGIC.length);
  ...
  async readRowGroup(schema, rowGroup, columnList) {
    ...
    for (let colChunk of columns) {
      ...
      let location = this.findColumnChunkLocation(schema, colChunk)
      locations.push(location)
    }

    let data = this._vf.readBatch(locations)

    // TODO reassert the assert :)
    assert(data.length === columns.length, 'expected data and columsn to have the same length')

    for (let i = 0; i < data.length; i++) {
      let columnData = await this.readColumnChunk(schema, columns[i], data[i])
      buffer.columnData[columnData.colKey] = columnData.payload;
    }

    return buffer;
  }

  async findColumnChunkLocation(schema, colChunk) {
      if (colChunk.file_path !== null) {
        ...
        let position = +colChunk.meta_data.data_page_offset;
        let length = +colChunk.meta_data.total_compressed_size;

        return { position, length }
  }

  async readColumnChunk(schema, colChunk, data) {
    return decodeDataPages(data, {
      type: type,
  ...
  async readFooter() {
    let trailerLen = PARQUET_MAGIC.lengthvf 4;
