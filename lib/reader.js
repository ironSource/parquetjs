...
function decodeDataPages(buffer, opts) {
  ...
  while (offset < buffer.length) {
    ...
    offset += parquet_util.decodeThrift(pageHeader, buffer);

    Array.prototype.push.apply(
        data,
        decodeDataPage(buffer, offset, pageHeader, opts));

    offset += pageHeader.compressed_page_size;
  }

  return data;
}

function decodeDataPage(buffer, offset, header, opts) {
  let rLevels = [];
  offset += parquet_codec_rle.decodeValues(
      "INT32",
      buffer,
      offset,
  ...
  switch (valueEncoding) {
    ...
    case 'PLAIN':
      parquet_codec_plain.decodeValues(
          opts.type,
          buffer,
          offset,
          valueCount,
    ...
    case 'RLE':
      parquet_codec_rle.decodeValues(
          opts.type,
          buffer,
          offset,
          valueCount,
    ...
    default:
      throw 'invalid encoding: ' + valueEncoding;

  }

  let valueIndex = 0;
  return dLevels.map((dlevel, i) => {
    let value = null;
    if (dlevel == opts.dLevelMax) {
