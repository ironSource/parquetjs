...
class ParquetEnvelopeReader {
  ...
  async readColumnChunk(colChunk) {
    ...
    return decodeDataPages(type, pagesBuf);
  }
...
function decodeDataPages(type, buffer) {
  let data = [];
  ...
  while (offset < buffer.length) {
    ...
    data += decodeDataPage(type, buffer, offset, pageHeader);
...
function decodeDataPage(type, buffer, offset, header) {
