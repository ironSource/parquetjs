...
class ParquetWriter {
  ...
  static async openFile(schema, path, opts) {
    ...
    let envelopeWriter = await ParquetEnvelopeWriter.openFile(schema, path, opts);
    try {
...
function encodeDataPage(column, valueCount, values, rlevels, dlevels) {
  ...
  pageHeader.type = parquet_util.setThriftEnum(parquet_thrift.PageType, 'DATA_PAGE');
...
function encodeDataPageV2(column, valueCount, rowCount, values, rlevels, dlevels) {
  ...
  /* build page header */
  let pageHeader = new parquet_thrift.PageHeader();
  pageHeader.type = parquet_util.setThriftEnum(parquet_thrift.PageType, 'DATA_PAGE_V2');
  pageHeader.data_page_header_v2 = new parquet_thrift.DataPageHeaderV2();
  pageHeader.data_page_header_v2.num_values = valueCount;
  pageHeader.data_page_header_v2.num_nulls = values.length;
  ...
  pageHeader.data_page_header_v2.definition_levels_byte_length =
      parquet_util.setThriftWorkaround(dLevelsBuf.length);

  pageHeader.data_page_header_v2.repetition_levels_byte_length =
      parquet_util.setThriftWorkaround(rLevelsBuf.length);
...
function encodeColumnChunk(values, opts) {
  ...
  metadata.type = parquet_util.setThriftEnum(
      parquet_thrift.Type,
      opts.column.primitiveType);

  metadata.codec = parquet_util.setThriftEnum(
...
function encodeFooter(schema, rowCount, rowGroups, userMetadata) {
...

  {
    ...
    schemaRoot.num_children = Object.keys(schema.column_map).length;
  ...
  for (let colDef of schema.columns) {
    ...
    schemaElem.repetition_type = parquet_util.setThriftEnum(
        ...
        colDef.repetitionType);

    if (colDef.isNested) {
      schemaElem.num_children = colDef.fieldCount;
    } else {
      schemaElem.type = parquet_util.setThriftEnum(
          parquet_thrift.Type,
          colDef.primitiveType);
    }

    if (colDef.originalType) {
      schemaElem.converted_type = parquet_util.setThriftEnum(
