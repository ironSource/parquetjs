...
var parquet_codec_plain = require('./encodings/plain')
var parquet_codec_rle = require('./encodings/rle')
...
class ParquetFileWriter {
  ...
  constructor(schema, path) {
    ...
    this.row_buffer = [];
    this.row_group_size = PARQUET_DEFAULT_ROW_GROUP_SIZE;
...
function ParquetWriter(schema, output_stream) {
  ...
  let row_count = 0;
  let row_groups = [];
  let page_size = PARQUET_DEFAULT_PAGE_SIZE;
...
function encodeDataPage(column, value_count, values, rlevels, dlevels) {
  ...
  switch (column.encoding) {
    ...
    case "PLAIN":
      values_buf = parquet_codec_plain.encodeValues(column.type, values);
      break;

    case "RLE":
      values_buf = parquet_codec_rle.encodeValues(column.type, values);
      break;
  ...
  let rlevels_buf = parquet_codec_rle.encodeValues(
      PARQUET_RDLVL_TYPE,
      rlevels,
      { bit_width: parquet_util.getBitWidth(column.rlevel_max) });

  let dlevels_buf = parquet_codec_rle.encodeValues(
      PARQUET_RDLVL_TYPE,
      dlevels,
...
function encodeFooter(schema, row_count, row_groups) {
...

  {
    ...
    metadata.schema.push(schema_root);
  }

  schema.columns.forEach(function(col_def) {
    let schema_elem = new parquet_thrift.SchemaElement();
    schema_elem.name = col_def.name;
    schema_elem.type = parquet_util.setThriftEnum(parquet_thrift.Type, col_def.type);
    schema_elem.repetition_type = parquet_util.setThriftEnum(
        parquet_thrift.FieldRepetitionType,
        col_def.repetition_type);

    metadata.schema.push(schema_elem);
  });

  let metadata_bin = parquet_util.serializeThrift(metadata);

  let footer = new Buffer(metadata_bin.length + 8);
  metadata_bin.copy(footer);
  footer.writeUInt32LE(metadata_bin.length, metadata_bin.length);
  footer.write(PARQUET_MAGIC, metadata_bin.length + 4);
  return footer;
}
