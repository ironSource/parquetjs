...
function ParquetWriter(schema, output_stream) {
  ...
  this.writeRowGroup = function(rows) {
    ...
    var rgroup = encodeRowGroup(schema, rows.length, rows_shredded, {
...
function encodeDataPage(column, value_count, values, rlevels, dlevels) {
  ...
  var rlevels_buf = codec_rle.encodeValues(
        "INT32",
        rlevels,
        { bit_width: parquet_util.getBitWidth(column.rlevel_max) });

  var dlevels_buf = codec_rle.encodeValues(
      "INT32",
      dlevels,
      { bit_width: parquet_util.getBitWidth(column.dlevel_max) });

  /* build page header */
  var page_body = Buffer.concat([rlevels_buf, dlevels_buf, values_buf]);
  ...
  page_header.definition_level_encoding =
    parquet_util.setThriftEnum(parquet_thrift.Encoding, "RLE");
...
function encodeRowGroup(schema, row_count, rows, opts) {
  var metadata = new parquet_thrift.RowGroup();
...
function encodeFooter(schema, row_count, row_groups) {
  ...
  var footer = new Buffer(fmd_bin.length + 8);
  fmd_bin.copy(footer);
