 ...
 */
const DEFAULT_PAGE_SIZE = 32; // FIXME
const DEFAULT_ROW_GROUP_SIZE = 50; // FIXME
...
function encodeValues(type, encoding, values) {
  switch (encoding) {
    ...
    case "RLE":
      return codec_rle.encodeValues(type, values);
...
function encodeColumnChunk(opts) {
  ...
  for (var voff = 0; voff <= values.length; voff += opts.page_size) {
    ...
    var page_values_buf = encodeValues(opts.column.type, page_encoding, page_values);

    var page_rlevels_buf = codec_rle.encodeValues(
        "INT32",
        page_rlevels,
        { bit_width: parquet_util.getBitWidth(opts.column.rlevel_max) });

    var page_dlevels_buf = codec_rle.encodeValues(
        "INT32",
        page_dlevels,
        { bit_width: parquet_util.getBitWidth(opts.column.dlevel_max) });

    /* build page  */
    var page_body = Buffer.concat([page_rlevels_buf, page_dlevels_buf, page_values_buf]);
    var page_header = new parquet_thrift.PageHeader()
    page_header.type = parquet_util.setThriftEnum(parquet_thrift.PageType, "DATA_PAGE");
    page_header.num_values = values.length;
    page_header.encoding = parquet_util.setThriftEnum(parquet_thrift.Encoding, page_encoding);
    page_header.definition_level_encoding = parquet_util.setThriftEnum(parquet_thrift.Encoding, "RLE");
    page_header.repetition_level_encoding = parquet_util.setThriftEnum(parquet_thrift.Encoding, "RLE");
  ...
  metadata.codec = parquet_util.setThriftEnum(parquet_thrift.CompressionCodec, pages_compression);
