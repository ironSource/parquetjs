...
function encodeColumnChunk(opts) {
  ...
  for (var voff = 0; voff <= values.length; voff += opts.page_size) {
    ...
    var page_body = Buffer.concat([
      codec_rle.encodeValues("INT32", page_rlevels, { bit_width: parquet_util.getBitWidth(opts.column.rlevel_max) }),
      codec_rle.encodeValues("INT32", page_dlevels, { bit_width: parquet_util.getBitWidth(opts.column.dlevel_max) }),
