...
class ParquetWriter {
  ...
  static async openFile(schema, path, opts) {
    ...
    try {
      ...
      return new ParquetWriter(schema, envelopeWriter);
    } catch (err) {
...
class ParquetEnvelopeWriter {
  ...
  static async openFile(schema, path, opts) {
    ...
    return new ParquetEnvelopeWriter(schema, writeFn, closeFn, 0);
  }
...
function encodeValues(type, encoding, values, opts) {
  switch (encoding) {
    ...
    case 'PLAIN':
      return parquet_codec_plain.encodeValues(type, values, opts);
      break;

    case 'RLE':
      return parquet_codec_rle.encodeValues(type, values, opts);
      break;
